...
from discord.ext import commands as ecommands
...
class AgeConverter(ecommands.Converter):
...
class AgeCalc(ecommands.Cog, command_attrs=dict(hidden=True)):
    def __init__(self, bot):
        self.bot = bot
        self.ext_path = 'age_bot.bot.cogs.age_calc'

    @ecommands.command(name='agecalc', aliases=['age'])
    async def agecalc(self, ctx: Context, age: AgeConverter):
        await ctx.reply(content=age, delete_after=60)

    @commands.command(name='agecalc', description='calculates age of users', guild_ids=[626522675224772658])
